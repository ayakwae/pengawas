<div class="row">
  <div class="col-md-12">
    <div class="container" style="background: white;font-color:blue;border-width: 1px;border-color: #d6d6c2;padding-top: 8px;padding-bottom: -1px;margin-top: 2.5em">
      <div class="row" style="margin-left: 5px;">
        <span style="background-color: #2F669F; height: 18px; width: 7px; margin-top: 7px; margin-left: 5px;"></span>
        <span style="background-color: #2F669F; height: 18px; width: 3px; margin-top: 7px; margin-left: 3px;"></span> &nbsp;
        <h4 style="color: #2F669F;"><b>Jadwal</b></h4>  
      </div>  
    </div>
  </div>
</div>
<div class="row">
  <div class="col-md-12">
    <div class="container" style="background: white;border-width: 1px;border-color: #d6d6c2;padding-bottom: 7px;">
      <div class="row" style="margin-left: 60px;" >
        </div>
      </div>
    </div>
  </div>

        <!-- isi disini tampilan honor -->


        <div class="row">
  <div class="col-md-12">
    <div class="container" style="background: white;border-width: 1px;border-color: #d6d6c2;padding-bottom: 10px; margin-top: 10px; padding-top: 25px;">
      <form><center>
        <table border="2" style="width:930px; text-align: center; margin-bottom: 20px;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">No</th>
            <th style="font-size: 25px; font-family: calibri;">&nbsp;&nbsp;&nbsp;&nbsp;Hari/Tanggal&nbsp;&nbsp;&nbsp;&nbsp;</th>
            <th style="font-size: 25px; font-family: calibri;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Jam&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
            <th style="font-size: 25px; font-family: calibri;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Mapel&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
            <th style="font-size: 25px; font-family: calibri;">Pengawas Ruang</th>
          </tr>

          <tr>
            <td>1.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>

              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>2.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>3.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>4.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>5.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>6.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>7.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>8.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>9.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>10.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
          <tr>
            <td>11.</td>
            <td>test</td>
            <td>test</td>
            <td>test</td>
            <td>
              <table border="0" style="width:500px; text-align: center;">
          <tr>
            <th style="font-size: 25px; font-family: calibri;">1</th>
            <th style="font-size: 25px; font-family: calibri;">2</th>
            <th style="font-size: 25px; font-family: calibri;">3</th>
            <th style="font-size: 25px; font-family: calibri;">4</th>
            <th style="font-size: 25px; font-family: calibri;">5</th>
            <th style="font-size: 25px; font-family: calibri;">6</th>
            <th style="font-size: 25px; font-family: calibri;">7</th>
            <th style="font-size: 25px; font-family: calibri;">8</th>
          </tr>
        </table>
            </td>
          </tr>
        </table>
      </center></form>
      <td style="padding-left: 230px;"><a href="<?php base_url() ?>halamanreport"><img src="<?php echo base_url()?>img\icon\keluar1.png" style="width: 50px; height: 40px;margin-left: 1010px;"></button></a></td>
    </div>
  </div>
</div>
      </div>
    </div>
  </div>
</div>